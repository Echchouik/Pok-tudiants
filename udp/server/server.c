...
static void app(void)
{
   ...
   while(1)
   {
	 ...
	 else
	 {
	    ...
	    send_back_message_sender(sock, clients, client, actual, buffer, 0);
	 }
      }
   }
...
static void send_back_message_sender(int sock, Client *clients, Client *sender, int actual, const char *buffer, char from_server)
{
   int i = 0;
...
{
   if(sendto(sock, "i am a poketudiant server", 25, 0, (SOCKADDR *) sin, sizeof *sin) < 0)
   {
      perror("send()");
      exit(errno);
   }
